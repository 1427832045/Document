package com.seer.srd.molex.statNew
...
import com.seer.srd.eventlog.VehicleStateLogger
import com.seer.srd.eventlog.recordSystemEventLog
import com.seer.srd.molex.CUSTOM_CONFIG
...
private val logger = LoggerFactory.getLogger("com.seer.srd.moshi.stat-new")
...
fun doStatDaily() {
    ...
    for (sa in statAccounts) {
        for (dayPartDef in emptyList<DayPartDef>()) {
...
private fun doStatJob(job: StatJob) {
    try {
        ...
        vehicles.forEach {
            ...
            if (account.type == "RobotTaskDurationAvg") pipeline.add(group(null, Accumulators.avg("value", "\$sum")))
