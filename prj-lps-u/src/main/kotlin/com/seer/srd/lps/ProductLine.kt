...
import com.seer.srd.lps.Services.getMsgOfBufferErrorCode
    ...
    private fun checkLineDownFill() {
        synchronized(downModbusHelper) {
            ...
            try {
                ...
                if (availableAgvSites.size < CUSTOM_CONFIG.leastDownSize) return

                val bufferAddress = CUSTOM_CONFIG.bufferAddress
                var matNum = downModbusHelper.read03HoldingRegisters(bufferAddress.matNumAddr, 1, 1, "${lineId}下料buffer数量")?.getShort(0)?.toInt()
                if (matNum is Int && matNum < 5) return
                var bufferOne = downModbusHelper.read03HoldingRegisters(bufferAddress.bufferOneAddr, 1, 1, "下料口1号位置")?.getShort(0)?.toInt()
                if (bufferOne == 0) return

                Thread.sleep(2000)
                matNum = downModbusHelper.read03HoldingRegisters(bufferAddress.matNumAddr, 1, 1, "${lineId}下料buffer数量")?.getShort(0)?.toInt()
                if (matNum is Int && matNum < 5) {
                    logger.debug("${lineId}下料buffer数量: $matNum，跳过此次下料任务创建")
                    return
                }
                bufferOne = downModbusHelper.read03HoldingRegisters(bufferAddress.bufferOneAddr, 1, 1, "下料口1号位置")?.getShort(0)?.toInt()
                if (bufferOne == 0) return

                val errorCode = downModbusHelper.read03HoldingRegisters(bufferAddress.errorCodeAddr, 1, 1, "$lineId down buffer error code")?.getShort(0)?.toInt()
                if (errorCode != 0) {
                    if (errorCode is Int && warningAlert["BufferError"] != true) {
                        warningAlert["BufferError"] = true
